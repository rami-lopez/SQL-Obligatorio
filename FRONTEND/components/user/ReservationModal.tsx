...
import { Room, User, ReservationParticipant, ParticipantStatus, ReservationStatus, Role, AttendanceStatus } from '../../types';
import { CalendarIcon } from '../icons';
import { AppContext } from '../../App';
import { createReservation, getParticipantSanctions } from '../../services/api';
...
export const ReservationModal: React.FC<ReservationModalProps> = ({ room, onClose }) => {
    ...
    const currentUserId = currentUser ? (currentUser.idParticipante ?? currentUser.id) : undefined;
    const [participants, setParticipants] = useState<ReservationParticipant[]>(currentUserId ? [{ idParticipante: currentUserId, participantId: currentUserId, estadoParticipacion: ParticipantStatus.CONFIRMADA, participationStatus: ParticipantStatus.CONFIRMADA, asistencia: AttendanceStatus.NO_REGISTRADO, attendance: AttendanceStatus.NO_REGISTRADO }] : []);
    ...
    const occupiedTurnIds = reservations
        .filter(r => (getReservationRoomId(r) === getRoomId(room)) && (r.fecha ?? r.date) === selectedDate && ((r.estado ?? r.status) === ReservationStatus.ACTIVA))
        .flatMap(r => {
            const start = r.start_turn_id ?? r.startTurnId ?? r.startTurnId ?? undefined;
    ...
    const participantUsers = participants.map(p => {
        ...
        return { ...p, name: user ? `${user.nombre ?? user.name} ${user.apellido ?? user.lastName}`: 'Usuario desconocido', user: user } as any;
    });

    const filteredUsers = users.filter(u =>
        `${u.nombre ?? u.name} ${u.apellido ?? u.lastName}`.toLowerCase().includes(searchTerm.toLowerCase()) &&
    ...
    const handleConfirm = async () => {
        ...
        for(let i=0; i < selectedTurnIds.length -1; i++){
            ...
            if (participants.length > (room.capacidad ?? room.capacity)) {
                alert(`El número de participantes (${participants.length}) excede la capacidad de la sala (${room.capacidad ?? room.capacity}).`);
            return;
        }
    ...
    const participantIds = participants.map(p => getUserId(p as any)).filter((x): x is number => typeof x === 'number');
        ...
        try {
            const createdReservation = await createReservation(newReservationData as any);
            setReservations([...reservations, createdReservation]);
            alert(`Reserva para ${room.nombre ?? room.name} confirmada. Se han enviado invitaciones a los participantes.`);
            onClose();
        } catch(error: any) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl transform transition-all max-h-[90vh] flex flex-col">
        <div className="p-6 border-b">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-2xl font-bold text-ucu-primary">Reservar Sala: {room.nombre ?? room.name}</h2>
              <p className="text-gray-500">Capacidad: {room.capacidad ?? room.capacity} personas</p>
            </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
            <div>
                <label htmlFor="date" className="block text-sm font-medium text-gray-700 mb-2">1. Seleccione la fecha</label>
                <div className="relative">
                    <input type="date" id="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} min={new Date().toISOString().split('T')[0]} className="w-full p-2 border border-gray-300 rounded-md focus:ring-ucu-secondary focus:border-ucu-secondary"/>
                    <CalendarIcon className="w-5 h-5 absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"/>
                </div>
            </div>
            
            <div>
                 <label className="block text-sm font-medium text-gray-700 mb-2">2. Seleccione los turnos (máx. 2)</label>
                        ...
                        return (
                            <button 
                                ...
                                className={`p-2 rounded-md text-sm text-center border transition-colors
                                    ${occupied ? 'bg-gray-200 text-gray-400 cursor-not-allowed line-through' : ''}
            ...
            <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">3. Añadir participantes ({participants.length}/{room.capacidad})</label>
                 <div className="flex flex-wrap gap-2 mb-2 p-2 border border-gray-300 rounded-md min-h-[44px] bg-gray-50">
                    {participantUsers.map(p => (
                        <span key={(p.id_participante ?? p.participantId)} title={(p.estado_participacion ?? p.participationStatus)} className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium shadow-sm ${((p.estado_participacion ?? p.participationStatus) === ParticipantStatus.CONFIRMADA) ? 'bg-ucu-primary text-white' : 'bg-ucu-light-blue text-ucu-primary'}`}>
                            {p.name}
                            {(p.id_participante ?? p.participantId) !== (currentUserId) && p.user && (
                                <button onClick={() => removeParticipant(p.user as User)} className="font-bold text-ucu-primary/70 hover:text-red-500 transition-colors">&times;</button>
                            )}
                        </span>
                    ))}
                </div>
                ...
                {participants.length < (room.capacidad ?? room.capacity) ? (
                    <div className="relative">
                        ...
                        {searchTerm && filteredUsers.length > 0 && (
                            <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-md mt-1 max-h-40 overflow-y-auto shadow-lg">
                                {filteredUsers.map(u => (
                                    <li key={(u.id_participante ?? u.id)} onClick={() => addParticipant(u)} className="px-4 py-2 hover:bg-ucu-light-gray cursor-pointer">
                    ...
                    <p className="text-sm text-center text-gray-500 p-2 bg-ucu-light-gray rounded-md">Capacidad máxima de la sala alcanzada.</p>
                )}
            </div>
        </div>
        ...
        <button onClick={handleConfirm} className="px-4 py-2 bg-ucu-secondary text-white rounded-md hover:bg-blue-600">
            Confirmar Reserva
        </button>}
        </div>
      </div>
